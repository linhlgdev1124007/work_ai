...
export function parseClassification(raw: string) {
  ...
  if (actionable && result.intent === 'CREATE_USER_NOTE' && (!result.data.note_content?.trim() || !result.data.target_user_name?.trim())) throw new Error('Missing note content or target user');
  if (actionable && result.intent === 'REPORT_TASK_DELAY' && !result.data.task_id) throw new Error('Missing task ID for delay report');
